...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        ## dict
        datapoint_dict = {"TP": [], "FN": [], "FP": [], "mislabel": []}
        ...
        for i, gc in enumerate(gt_classes):
            ...
            if n and sum(j) == 1:
                ...
                if detection_classes[m1[j]] == gc:
                    datapoint_dict["TP"].append(
                        {"label": labels[i], "pred": detections[m1[j][0]]}
                    )
                else:
                    datapoint_dict["mislabel"].append(
                        {"label": labels[i], "pred": detections[m1[j][0]]}
                    )

            else:
                self.matrix[self.nc, gc] += 1  # true background
                datapoint_dict["FN"].append({"label": labels[i], "pred": None})

        if n:
            for i, dc in enumerate(detection_classes):
                if not any(m1 == i):
                    self.matrix[dc, self.nc] += 1  # predicted background
                    datapoint_dict["FP"].append({"label": None, "pred": detections[i]})

        return self.matrix, datapoint_dict
