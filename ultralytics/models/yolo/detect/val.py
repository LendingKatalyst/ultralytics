...
class DetectionValidator(BaseValidator):
    ...
    def return_results_str(self):
        import io

        """Prints training/validation set metrics per class."""
        pf = "%22s" + "%11i" * 2 + "%11.3g" * len(self.metrics.keys)  # print format
        output_string = io.StringIO()  # Initialize the StringIO object
        output_string.write(
            pf
            % ("all", self.seen, self.nt_per_class.sum(), *self.metrics.mean_results())
            + "\n"
        ...
        # Get the complete text from the StringIO object
        result_text = output_string.getvalue()
        output_string.close()  # Close the StringIO object
